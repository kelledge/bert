  ...
  def run(self):
    ...
    while True:
      ...
      if self.frame is False:
        logger.info('test="' + self.test_name + '" result="fail" reason="response timeout"')
      elif self.frame.valid is True:
        logger.info('test="' + self.test_name + '" result="pass" reason="valid checksum"')
        
      else:
        logger.info('test="' + self.test_name + '" result="fail" reason="invalid checksum"')
      time.sleep(59)
      
class SerialTest(object):
  def __init__(self, serial_number, serial_type):
    handler.setFormatter(logging.Formatter(fmt=(serial_number + ': %(message)s')))
